...
def sat_3_coloring(G):
    ...
    for v in range(G.N):
        solver.add_clause([toBase3(v, 0), toBase3(v, 1), toBase3(v, 2)])

    #enpoints can't have same color
    for vertex in G.edges:
        for edge in vertex:
            v = G.edges.index(vertex)
            solver.add_clause([-1 * toBase3(v, 0), -1 * toBase3(edge, 0)])
            solver.add_clause([-1 * toBase3(v, 1), -1 * toBase3(edge, 1)])
            solver.add_clause([-1 * toBase3(v, 2), -1 * toBase3(edge, 2)])
